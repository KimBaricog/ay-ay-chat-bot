...
function send() {
  if (input.value.trim() === "") {
    return;
  } else if (input.value && imgIput.value === "") {
  ...
  //img send message
  if (imgIput.value && input.value) {
    let imgRply = ["kim gwapo"];
    setTimeout(() => {
      let randomIndex1 = Math.floor(Math.random() * Ai.length);
      botdiv.innerHTML = `
            <p>${imgRply[randomIndex1]}</p>`;
      chatContainer.scrollTop = chatContainer.scrollHeight;
    }, 3000);
  }
}
